  ...
  //  $imgUpload=$_FILES['imgUpload'];//รับค่า FILE
    ...
    $url = '../../../../assets/image/store/'. $new_name ;
        if(isset($product_image) != $imgUpload_file){ //ค่าเหมือนกัน เช่น aa.jpg aa.jpg
             $sql="UPDATE `product` SET `product_name` = '$product_name', `product_detail` = '$product_detail', `product_price` = '$product_price', `product_discount` = '$product_discount', `product_code` = '$product_code', `product_tag` = '$product_tag', `product_date` = '$product_date' WHERE `product_id`= '$product_id'";
            ...
            if ( move_uploaded_file($_FILES['imgUpload']['tmp_name'], $url ) ) {
                $sql="UPDATE `product` SET `product_name` = '$product_name',`product_image`='$product_image', `product_detail` = '$product_detail', `product_price` = '$product_price', `product_discount` = '$product_discount', `product_code` = '$product_code', `product_tag` = '$product_tag', `product_date` = '$product_date' WHERE `product_id`= '$product_id'";
                $result = $conn->query($sql) or die($conn->error);
                if($result){    
